...
dm9000_probe(struct platform_device *pdev)
{
	...
	unsigned char ne_def_eth_mac_addr[]={0x1a,0x2b,0x3c,0x4d,0x5e,0x6f}; 
    static void *bwscon;  
    static void *gpgcon;  
    static void *gpgup;  
    static void *extint1;  
    ...
    #define INTMSK           (0x4A000008)  
        ...
        gpgup=ioremap_nocache(GPGUP,0x0000004); 
        extint1=ioremap_nocache(EXTINT1,0x0000004);  
        intmsk=ioremap_nocache(INTMSK,0x0000004);  
                         
        //writel(readl(bwscon)|0xc0000,bwscon); 
        writel(readl(bwscon)|0x0c000,bwscon);  
        //writel( (readl(gpgcon) & ~(0x3 << 14)) | (0x2 << 14), gpgcon);  
        writel( (readl(gpgcon) & ~(0x3 << 2)) | (0x2 << 2), gpgcon);  //modify bu sunny
        //writel( readl(gpgcon) | (0x1 << 7), gpgcon); // Disable pull-up  
        writel( readl(gpgup) | (0x1 << 1), gpgup); // Disable pull-up  
        //writel( (readl(extint1) & ~(0xf << 28)) | (0x4 << 28), extint1); //rising edge  
        writel( (readl(extint1) & ~(0xf << 4)) | (0x4 << 4), extint1); //rising edge
        //writel( (readl(intmsk))  & ~0x80, intmsk);   
        writel( (readl(intmsk))  & ~0x20, intmsk);  
	// -----------------------------------------------------------------------
